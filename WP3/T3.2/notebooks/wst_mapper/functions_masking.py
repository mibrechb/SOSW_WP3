...
def apply_qa_cloudmask(img):
    ...
    opt_masked = img.select('SR_.*').updateMask(isCloud.Not())
    st_masked = img.select('SR_.*').updateMask(isCloud.Not())
    return img \
...
def apply_swm_watermask(im):
    ...
    is_water = index_criteria.rename('is_water')
